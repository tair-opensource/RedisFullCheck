...
var opts struct {
	...
	CompareMode    int    `short:"m" long:"comparemode" default:"2" description:"compare mode, 1: compare full value, 2: only compare value length, 3: only compare keys outline"`
...
/*
func argsParse(args []string, hosts *[]RedisHost) error {

	if len(args) != 2 {
		return fmt.Errorf("specified host num(ip:port@password) must be 2, one for source, the other for dest")
	}

	for i := 0; i < len(args); i++ {
		var host RedisHost
		pos := strings.IndexByte(args[i], '@')
		if pos == -1 {
			host.Addr = args[i]
		} else {
			host.Addr = string(args[i][0:pos])
			host.Password = string(args[i][pos+1:])
		}
...
func main() {
	...
	// var redisHosts []RedisHost
	// err = argsParse(args, &redisHosts)
	// if err != nil {
	// 	fmt.Println(err)
	// 	os.Exit(1)
	// }
	...
	if opts.CompareMode < FullValue || opts.CompareMode > KeyOutline {
